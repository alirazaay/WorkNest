...
function ContinuityWorkspace({ data, onRetry, setData, canManage }) {
  ...
  return <div className="continuity-workspace">
    <article className="performance-card">
      ...
      {message && <p className="performance-inline-message">{message}</p>}
      {history && history.summary?.yearsReviewed === 0 && <p className="performance-inline-message">No finalized performance history is available yet.</p>}
      {loadingEmployee ? <LoadingState label="Loading employee history..." /> : history && history.summary?.yearsReviewed > 0 ? <>
        ...
        <div className="continuity-timeline">{(history.timeline || []).map((row) => <div className={`continuity-year ${row.status === 'no_review_data' ? 'missing' : ''}`} key={row.year}><strong>{row.year}</strong><span>{row.originalRating == null ? 'No Review Data' : `${row.originalRating}/5`}</span>{row.finalScore != null && <small>{row.cycleName || 'Finalized cycle'} · Final score: {row.finalScore.toFixed(2)}{row.ratingBand ? ` · ${row.ratingBand}` : ''}</small>}<StatusBadge status={row.trend === 'insufficient_history' ? 'neutral' : row.trend} /><small>{row.changeFromPreviousYear == null ? 'Baseline / insufficient history' : `${row.changeFromPreviousYear > 0 ? '+' : ''}${row.changeFromPreviousYear} from previous year`}</small></div>)}</div>
...
function CalibrationWorkspace({ items, cycles, cycle, canAdmin, onRetry }) {
  ...
  async function reopen(reviewId) { const reason = window.prompt('Correction reason (minimum 5 characters):'); if (!reason || reason.trim().length < 5) return; try { await api.post(`/performance/reviews/${reviewId}/reopen`, { reason: reason.trim() }); setMessage('Review reopened as a draft revision. The original confirmed version is preserved.'); await onRetry(); } catch (err) { setMessage(err.response?.data?.error?.message || 'Unable to reopen review for correction.'); } }
  return <article className="performance-card"><div className="performance-card-heading"><div><h2>Calibration</h2><p>Review submitted appraisals before FairRank calculation and release.</p></div></div><label className="comparison-cycle">Cycle<select value={cycleId} onChange={selectCycle}><option value="">Select cycle</option>{cycles.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label>{message && <p className="performance-inline-message">{message}</p>}{rows.length ? <div className="performance-list">{rows.map((item) => { const review = item.review || item; const decisionStatus = item.calibrationDecision?.status; const isConfirmed = decisionStatus === 'confirmed'; const displayStatus = isConfirmed ? 'confirmed' : decisionStatus === 'correction_required' ? 'correction required' : decisionStatus === 'clarification_requested' ? 'clarification requested' : review.status; const score = review.overallScore ?? item.scoreSnapshot?.finalScore; return <article className="performance-list-card" key={review.id}><div><strong>{review.employee?.user?.name || `Review #${review.id}`}</strong><small>{titleCase(review.reviewType)} · score {score ?? 'Pending FairRank calculation'} · {titleCase(displayStatus)}</small></div><div><StatusBadge status={displayStatus} />{canAdmin && isConfirmed && <button className="table-link" onClick={() => reopen(review.id)}>Reopen for Correction</button>}{canAdmin && !isConfirmed && ['submitted', 'in_progress'].includes(review.status) && <><button className="table-link" onClick={() => decide(review.id, 'confirm')}>Confirm</button><button className="table-link" onClick={() => decide(review.id, 'request_clarification')}>Clarify</button></>}</div></article>; })}</div> : <EmptyState text="No submitted reviews are available for calibration." />}</article>;
}
...
function ActionWorkspace({ tab, items, cycles, employees, criteria, profiles, onRetry, canManage, canAdmin, canSelfServe }) {
  ...
  const editingReview = tab === 'reviews' ? items.find((row) => String(row.id) === String(editId)) : null;
  ...
  function editReview(item) { setEditId(item.id); setReviewCriteria((item.scores || []).map((score) => ({ ...(score.criterion || {}), id: score.criterionId, rawScore: score.rawScore }))); setFormError(''); setModal(true); }
  ...
  async function submit(event) {
    ...
    try {
      ...
      if (tab === 'reviews' && editId) await api.patch(`/performance/reviews/${editId}`, { strengths: payload.strengths, improvementAreas: payload.improvementAreas, comments: payload.comments, scores: payload.scores });
      else await api.post(route, payload);
      setModal(false); setProfileId(''); setEditId(''); setMessage(`${title} ${tab === 'reviews' && editId ? 'revision' : 'record'} saved successfully.`); await onRetry();
    } catch (err) {
  ...
  return <article className="performance-card">
    ...
    {modal && <div className="modal-backdrop"><form className="modal performance-modal" onSubmit={submit}><button type="button" className="modal-close" onClick={() => { setModal(false); setEditId(''); }}>×</button><h2>{editingReview ? 'Edit draft review revision' : `Add ${title.slice(0, -1)}`}</h2><p>{editingReview ? 'Update the corrected scores and comments, then resubmit for calibration.' : 'Save this step to continue the FairRank workflow.'}</p>
      ...
      {tab === 'reviews' && <><RecordSelectors cycles={cycles} employees={employees} onCycleChange={loadReviewCriteria} defaultCycleId={editingReview?.cycleId} defaultEmployeeId={editingReview?.employeeId} /><label>Review type<select name="reviewType" defaultValue={editingReview?.reviewType || (canManage ? 'manager' : 'self')}><option value="self">Self</option>{canManage && <option value="manager">Manager</option>}</select></label><fieldset><legend>Criterion scores (0–100)</legend>{criteriaLoading ? <p className="performance-inline-message">Loading criteria for this cycle…</p> : reviewCriteria.length ? reviewCriteria.map((criterion) => <label key={criterion.id}>{criterion.name}<input name={`score_${criterion.id}`} type="number" min="0" max="100" defaultValue={editingReview ? criterion.rawScore : undefined} /></label>) : <p className="performance-inline-message">Select a cycle to load its template criteria.</p>}</fieldset><label>Comments<textarea name="comments" defaultValue={editingReview?.comments || ''} /></label></>}
