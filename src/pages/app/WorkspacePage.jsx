...
export default function WorkspacePage({ user, onExit }) {
  ...
  const employeeColumns = [{ key: 'name', label: 'Employee', render: row => row.user?.name || row.name }, { key: 'email', label: 'Email', render: row => row.user?.email || row.email }, { key: 'designation', label: 'Role' }, { key: 'status', label: 'Status', render: row => <StatusBadge status={row.employmentStatus || row.status} /> }];
  const firstName = user?.user?.name?.split(' ')[0] || 'Amara';
  return <div className="app"><Sidebar active={section} onNavigate={label => navigate({ Overview: '/dashboard', People: '/employees', Attendance: '/attendance', 'Time off': '/leaves', Payroll: '/payroll', Departments: '/departments', Settings: '/settings' }[label] || '/dashboard')} onLogout={onExit} /><div className="app-content"><Topbar user={user} onNotifications={() => notify('You are all caught up')} /><main className="dashboard-main">{error && <ErrorState message={error} onRetry={loadRecords} />}{!error && section === 'Departments' && <><Breadcrumbs items={[{ label: 'Workspace' }, { label: 'Departments' }]} /><div className="page-heading"><div><div className="eyebrow">Workspace / Manage</div><h1>Departments</h1><p>Shape the teams and spaces that make your organization work.</p></div><Button size="sm" onClick={() => setModal('department')}><span>＋</span> Add department</Button></div><div className="department-grid">{departments.map(department => <div className="department-card" key={department.id}><div className="department-icon">⌘</div><h2>{department.name}</h2><p>Team space for your organization.</p><StatusBadge status="active">Active</StatusBadge></div>)}</div></>}{!error && section === 'People' && <><Breadcrumbs items={[{ label: 'Workspace' }, { label: 'People' }]} /><div className="page-heading"><div><div className="eyebrow">Workspace</div><h1>People</h1><p>Manage your organization’s employee directory.</p></div><Button size="sm" onClick={() => setModal('employee')}>＋ Add employee</Button></div><DataTable columns={employeeColumns} rows={employees} loading={loading} emptyTitle="No employees found" /></>}{!error && !['Departments', 'People'].includes(section) && <><Breadcrumbs items={[{ label: 'Workspace' }, { label: section }]} /><div className="page-heading"><div><div className="eyebrow">Monday, April 14, 2025</div><h1>Good morning, {firstName} <span className="wave">✦</span></h1><p>Here’s what’s happening across your workspace today.</p></div><Button size="sm" onClick={() => setModal('employee')}>＋ Add employee</Button></div><div className="kpi-grid"><div className="kpi-card"><div className="kpi-icon violet">♙</div><small>Total people</small><strong>{loading ? '—' : employees.length}</strong><span className="positive">From database</span></div></div></>}</main></div><Modal open={Boolean(modal)} title={modal === 'department' ? 'Add department' : 'Add employee'} description={modal === 'department' ? 'Create a new team space for your organization.' : 'Add a person to your workspace.'} onClose={() => setModal(null)}><form onSubmit={submit}><label>{modal === 'department' ? 'Department name' : 'Full name'}<input name="name" required autoFocus placeholder={modal === 'department' ? 'e.g. Product Design' : 'e.g. Jordan Alvarez'} /></label>{modal === 'employee' && <><label>Work email<input name="email" type="email" required placeholder="jordan@company.com" /></label><label>Password<input name="password" type="password" minLength="8" required placeholder="Temporary password" /></label><label>Job title<input name="role" placeholder="e.g. Product Designer" /></label></>}<Button className="full" loading={false}>{modal === 'department' ? 'Add department' : 'Add employee'} <span>→</span></Button></form></Modal><Toast message={message} onClose={() => setMessage('')} /></div>;
}
