...
export default function Sidebar({ active, role = 'admin', companyName: providedCompanyName, open = false, onNavigate, onLogout, onClose }) {
  ...
  return <><div className={open ? 'sidebar-scrim visible' : 'sidebar-scrim'} onClick={onClose} /><aside className={open ? 'app-sidebar mobile-open' : 'app-sidebar'}><div className="sidebar-brand"><Logo /><button type="button" className="sidebar-close" onClick={onClose} aria-label="Close menu"><X size={18} /></button><span className="workspace">{companyName}</span></div><div className="sidebar-nav" aria-label="Workspace navigation"><div className="sidebar-label">Workspace</div>{links.map(([label, Icon]) => <button type="button" key={label} className={active === label ? 'side-link active' : 'side-link'} aria-current={active === label ? 'page' : undefined} onClick={() => { onNavigate(label); onClose?.(); }}><Icon size={17} />{label === 'Time off' ? 'Time Off' : label}</button>)}</div><div className="sidebar-bottom"><button type="button" className="side-link sidebar-signout" onClick={onLogout}><LogOut size={17} />Sign Out</button></div></aside></>;
}
