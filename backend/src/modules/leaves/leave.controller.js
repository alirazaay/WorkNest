...
export async function typesCreate(req, res, next) { try { send(res, await createLeaveType(req.auth.tenantId, req.validated.body), 201); } catch (error) { next(error); } }
...
export async function requestsList(req, res, next) { try { send(res, await listLeaveRequests(req.auth, req.validated.query)); } catch (error) { next(error); } }
...
export async function requestCreate(req, res, next) { try { send(res, await createLeaveRequest(req.auth, req.validated.body), 201); } catch (error) { next(error); } }
