...
export async function listNotifications(auth, query = {}) {
  const limit = Math.min(100, Math.max(1, Number(query.limit || 30))); const where = { tenantId: auth.tenantId, userId: auth.userId }; const [items, unreadCount] = await Promise.all([Notification.findAll({ where, order: [['created_at', 'DESC']], limit }), Notification.count({ where: { ...where, isRead: false } })]); return { items, unreadCount };
}
