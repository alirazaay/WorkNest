...
export const performanceGoalCarryForwardSchema = z.object({ targetCycleId: z.coerce.number().int().positive(), title: z.string().min(3).max(180).optional(), description: z.string().max(2000).optional().nullable(), dueDate: date.optional().nullable() });
const listPaging = { page: z.coerce.number().int().positive().optional(), pageSize: z.coerce.number().int().positive().max(100).optional() };
export const performanceGoalQuerySchema = z.object({ cycleId: z.coerce.number().int().positive().optional(), employeeId: z.coerce.number().int().positive().optional(), status: z.enum(goalStatuses).optional(), ...listPaging });
...
export const performanceEvidenceQuerySchema = z.object({ cycleId: z.coerce.number().int().positive().optional(), employeeId: z.coerce.number().int().positive().optional(), goalId: z.coerce.number().int().positive().optional(), criterionId: z.coerce.number().int().positive().optional(), verificationStatus: z.enum(['pending', 'verified', 'rejected']).optional(), ...listPaging });
...
export const performanceReviewUpdateSchema = z.object({ strengths: z.string().max(5000).optional().nullable(), improvementAreas: z.string().max(5000).optional().nullable(), comments: z.string().max(5000).optional().nullable(), scores: z.array(reviewScoreSchema).min(1).max(100) }).superRefine((value, ctx) => { const ids = value.scores.map(score => score.criterionId); if (new Set(ids).size !== ids.length) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['scores'], message: 'Each criterion may only be scored once per review' }); });
export const performanceReviewCorrectionSchema = z.object({ reason: z.string().trim().min(5).max(3000) });
export const performanceReviewQuerySchema = z.object({ cycleId: z.coerce.number().int().positive().optional(), employeeId: z.coerce.number().int().positive().optional(), reviewerId: z.coerce.number().int().positive().optional(), reviewType: z.enum(reviewTypes).optional(), status: z.enum(reviewStatuses).optional(), ...listPaging });
...
export const performanceRewardQuerySchema = z.object({ cycleId: z.coerce.number().int().positive().optional(), employeeId: z.coerce.number().int().positive().optional(), rewardType: z.enum(rewardTypes).optional(), status: z.enum(['recommended', 'approved', 'rejected']).optional(), ...listPaging });
...
export const trainingNeedUpdateSchema = trainingNeedCreateSchema.omit({ employeeId: true }).partial();
export const tnaAnalysisSchema = z.object({ fromYear: z.coerce.number().int().min(2000).max(2200).optional(), toYear: z.coerce.number().int().min(2000).max(2200).optional(), lowRatingThreshold: z.coerce.number().min(1).max(5).default(2) });
