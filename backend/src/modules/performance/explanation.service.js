...
export function buildPerformanceExplanation({ snapshot, review, evidenceCoverage, signature, equivalenceGroup, promotionAssessment, promotionProfile }) {
  const score = Number(snapshot.finalScore).toFixed(3); const band = snapshot.ratingBand || 'unassigned rating band'; const coverage = Number(snapshot.evidenceCoveragePercentage ?? evidenceCoverage);
  const performanceConclusion = `Annual performance is ${score} points and is classified as ${band}. The result is based on the stored criterion-level score breakdown and supporting evidence coverage of ${coverage.toFixed(2)}%.`;
  ...
  return { finalScore: Number(snapshot.finalScore), ratingBand: snapshot.ratingBand, performanceConclusion, promotionConclusion, criterionBreakdown: snapshot.calculationDetails?.lines || [], evidenceCoveragePercentage: Number(snapshot.evidenceCoveragePercentage ?? evidenceCoverage), evidenceConfidence: snapshot.evidenceConfidence || snapshot.calculationDetails?.evidenceConfidence || 'low', equivalenceConclusion, performanceSignature: signature?.signatureName ?? null, explanationSnapshot: { version: 1, sourceSnapshotId: snapshot.id, sourceReviewId: review?.id ?? null, sourceSignatureId: signature?.id ?? null, sourcePromotionAssessmentId: promotionAssessment?.id ?? null, generatedAt: new Date().toISOString() } };
}
