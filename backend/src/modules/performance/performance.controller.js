...
import { createReview, getReview, listCycleReviewCriteria, listReviews, reopenReviewForCorrection, submitReview, updateReview } from './reviews.service.js';
...
export async function reviewUpdate(req, res, next) { try { send(res, await updateReview(req.auth, Number(req.params.id), req.validated.body)); } catch (error) { next(error); } }
export async function reviewReopen(req, res, next) { try { send(res, await reopenReviewForCorrection(req.auth, Number(req.params.id), req.validated.body)); } catch (error) { next(error); } }
