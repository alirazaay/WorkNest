...
import { Bonus, Employee, EmployeeBankAccount, EmployeeDeduction, EmployeeSalaryComponent, EmployeeSalaryStructure, EmployeeTaxConfiguration, LeaveRequest, LeaveType, LoanInstallment, Notification, PayrollItem, PayrollItemLine, PayrollRun, SalaryComponent, Tenant, User } from '../../database/models/index.js';
...
async function calculationFor(employee, salary, month, year, period, unpaidLeaveDays, transaction) {
  ...
  const taxConfig = await EmployeeTaxConfiguration.findOne({ where: { tenantId: employee.tenantId, employeeId: employee.id, isActive: true, ...periodWhere(period.start, period.end) }, order: [['effectiveFrom', 'DESC']], transaction });
  if (taxConfig) taxCents = taxConfig.calculationType === 'percentage' ? Math.round(base * Number(taxConfig.percentage || 0) / 100) : cents(taxConfig.amount);
...
const runInclude = [{ model: PayrollItem, as: 'items', include: [activeEmployeeInclude(), { model: PayrollItemLine, as: 'lines' }] }, { model: User, as: 'generator', attributes: ['id', 'name'] }, { model: User, as: 'approver', attributes: ['id', 'name'] }, { model: Tenant, as: 'tenant', attributes: ['id', 'companyName', 'logoUrl', 'address'] }];
...
export async function bankCsv(auth, id, format = 'standard') { const run = await getPayrollRun(auth, id); if (!['approved', 'locked'].includes(run.status)) throw new AppError('Bank export requires approved or locked payroll', 409, 'INVALID_PAYROLL_STATUS'); const standard = run.items.map((item) => { const snapshot = item.bankSnapshot || {}; return { code: item.employee.employeeCode, name: item.employee.user.name, bank: snapshot.bankName || '', title: snapshot.accountTitle || '', account: snapshot.accountNumber || '', iban: snapshot.iban || '', amount: item.netSalary, month: run.month, year: run.year, reference: `WORKNEST-${run.year}${String(run.month).padStart(2, '0')}-${item.id}` }; }); const layouts = { standard: { headers: ['Employee Code', 'Employee Name', 'Bank Name', 'Account Title', 'Account Number', 'IBAN', 'Net Salary', 'Payroll Month', 'Payroll Year', 'Payment Reference'], fields: ['code', 'name', 'bank', 'title', 'account', 'iban', 'amount', 'month', 'year', 'reference'] }, hbl: { headers: ['Account Number', 'Account Title', 'IBAN', 'Amount', 'Reference'], fields: ['account', 'title', 'iban', 'amount', 'reference'] }, meezan: { headers: ['IBAN', 'Account Title', 'Amount', 'Payment Reference'], fields: ['iban', 'title', 'amount', 'reference'] }, ubl: { headers: ['Employee Code', 'IBAN', 'Amount', 'Narration'], fields: ['code', 'iban', 'amount', 'reference'] } }; const layout = layouts[format] || layouts.standard; const rows = [layout.headers.join(',')]; for (const item of standard) rows.push(layout.fields.map((field) => csv(item[field])).join(',')); await recordAudit({ tenantId: auth.tenantId, actorUserId: auth.userId, action: `payroll_bank_exported_${format}`, entityType: 'payroll_run', entityId: id }); return rows.join('\n'); }
