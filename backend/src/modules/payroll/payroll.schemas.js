...
export const taxConfigurationSchema = z.object({ calculationType: z.enum(['fixed', 'percentage']).default('fixed'), amount: money.default(0), percentage: z.number().nonnegative().max(100).optional(), effectiveFrom: date, effectiveTo: date.nullable().optional(), isActive: z.boolean().default(true) });
export const payrollAdjustmentSchema = z.object({ payrollRunId: id, employeeId: id, lineType: z.enum(['earning', 'deduction']), amount: money, reason: z.string().min(3).max(1000) });
export const bankExportSchema = z.object({ format: z.enum(['standard', 'hbl', 'meezan', 'ubl']).default('standard') });
