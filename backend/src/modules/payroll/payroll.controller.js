...
export async function generateHandler(req, res, next) { try { send(res, await generatePayroll(req.auth, req.body), 201); } catch (error) { next(error); } }
export async function runsList(req, res, next) { try { send(res, await listPayrollRuns(req.auth, req.query)); } catch (error) { next(error); } }
...
export async function myList(req, res, next) { try { send(res, await getMyPayroll(req.auth, req.query)); } catch (error) { next(error); } }
export async function csvExport(req, res, next) { try { const id = Number(req.query.runId || req.params.id); const csv = await payrollCsv(req.auth, id); res.type('text/csv').attachment(`worknest-payroll-${id}.csv`).send(csv); } catch (error) { next(error); } }
