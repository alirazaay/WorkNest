...
export async function updateCompany(req, res, next) { try { const tenant = await Tenant.findByPk(req.auth.tenantId); Object.assign(tenant, req.validated.body); await tenant.save(); send(res, { company: tenant, workHours: await TenantSetting.findOne({ where: { tenantId: req.auth.tenantId } }) }); } catch (error) { next(error); } }
export async function updateWorkHours(req, res, next) { try { const [settings] = await TenantSetting.findOrCreate({ where: { tenantId: req.auth.tenantId }, defaults: { tenantId: req.auth.tenantId } }); Object.assign(settings, req.validated.body); await settings.save(); send(res, { company: await Tenant.findByPk(req.auth.tenantId), workHours: settings }); } catch (error) { next(error); } }
