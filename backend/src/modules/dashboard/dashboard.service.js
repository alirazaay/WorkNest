...
export async function activity(auth, query) {
  ...
  const [leaves, employees, payroll] = await Promise.all([
    LeaveRequest.findAll({ where: { tenantId: auth.tenantId, ...(ids ? { employeeId: { [Op.in]: ids } } : {}) }, include: [{ model: Employee, as: 'employee', include: [{ model: User, as: 'user', attributes: ['name'] }] }, { model: LeaveType, as: 'leaveType', attributes: ['name'] }], order: [['createdAt', 'DESC']], limit }),
    Employee.findAll({ where: employeeWhere, include: [{ model: User, as: 'user', attributes: ['name'] }], order: [['createdAt', 'DESC']], limit }),
    PayrollRun.findAll({ where: { tenantId: auth.tenantId }, order: [['createdAt', 'DESC']], limit })
  ]);
  return [...leaves.map((item) => ({ type: 'leave_request', id: item.id, message: `${item.employee?.user?.name || 'Employee'} submitted ${item.leaveType?.name || 'leave'} request`, status: item.status, createdAt: item.createdAt })), ...employees.map((item) => ({ type: 'employee_added', id: item.id, message: `${item.user?.name || 'Employee'} joined the workspace`, status: item.employmentStatus, createdAt: item.createdAt })), ...payroll.map((item) => ({ type: 'payroll_run', id: item.id, message: `Payroll for ${item.month}/${item.year} is ${item.status}`, status: item.status, createdAt: item.createdAt }))].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).slice(0, limit);
}
