...
export async function departmentsCreate(req, res, next) { try { send(res, await createDepartment(req.auth.tenantId, req.body), 201); } catch (error) { next(error); } }
export async function departmentsUpdate(req, res, next) { try { send(res, await updateDepartment(req.auth.tenantId, Number(req.params.id), req.body)); } catch (error) { next(error); } }
...
export async function employeesCreate(req, res, next) { try { send(res, await createEmployee(req.auth.tenantId, req.body), 201); } catch (error) { next(error); } }
export async function employeesUpdate(req, res, next) { try { send(res, await updateEmployee(req.auth, Number(req.params.id), req.body)); } catch (error) { next(error); } }
export async function employeesStatus(req, res, next) { try { send(res, await updateEmployeeStatus(req.auth, Number(req.params.id), req.body)); } catch (error) { next(error); } }
...
export async function salaryCreate(req, res, next) { try { send(res, await addSalaryStructure(req.auth, Number(req.params.id), req.body), 201); } catch (error) { next(error); } }
export async function documentCreate(req, res, next) { try { if (!req.file) return res.status(422).json({ success: false, error: { code: 'FILE_REQUIRED', message: 'A document file is required' } }); send(res, await addDocument(req.auth, Number(req.params.id), req.file, req.body.documentType), 201); } catch (error) { if (req.file?.path) await unlink(req.file.path).catch(() => {}); next(error); } }
