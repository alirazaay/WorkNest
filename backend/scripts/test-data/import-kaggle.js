...
importData().catch(error => { console.error(`Test-data import failed: ${error.message}`); if (error.errors) console.error(error.errors.map(item => ({ message: item.message, path: item.path, value: item.value }))); if (process.env.DEBUG_TEST_DATA_SEED === 'true') console.error(error.stack); process.exitCode = 1; }).finally(() => sequelize.close());
