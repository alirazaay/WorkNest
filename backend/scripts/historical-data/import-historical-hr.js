...
import { Department, Employee, EmployeeHistoryEvent, HistoricalPerformanceRecord, PerformanceCycle, PerformanceCycleLink, Tenant, TenantSetting, User } from '../../src/database/models/index.js';
...
async function importData() {
  ...
  await PerformanceCycleLink.bulkCreate(years.slice(1).map((year, index) => ({ tenantId: tenant.id, previousCycleId: cycles[years[index]].id, currentCycleId: cycles[year].id, linkType: 'year_over_year', metadata: { source: SOURCE })));
