...
### FairRank review creation

Reviews are created with `POST /api/v1/performance/reviews` and initially persist as `draft`. The Add Review form loads criteria from the selected cycle's active performance-template assignments through `GET /api/v1/performance/cycles/:cycleId/review-criteria`; it does not use the global criteria list. After saving scores and comments, the reviewer submits the draft with `POST /api/v1/performance/reviews/:id/submit`, making it available to the calibration workflow. Template assignment uniqueness, tenant scoping, duplicate-review protection, audit logging, and transaction-safe response loading are enforced by the backend.
